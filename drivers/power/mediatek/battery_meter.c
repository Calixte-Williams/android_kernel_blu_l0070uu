...
long _g_bat_sleep_total_time_ns;
...
#ifndef LOWPRE_FIX_RESET_FG
#define LOWPRE_FIX_RESET_FG 0
#endif
...
kal_int32 fgauge_update_dod(void)
{
...
#endif
	static kal_uint32 timer_ct = 1; //
	static kal_uint32 FG_dod_pre = 0; //
	...
	//FG_dod_1 = gFG_DOD0 - ((gFG_columb * 100) / gFG_BATT_CAPACITY_aging);
        ...
        #if LOWPRE_FIX_RESET_FG
        if (gFG_Is_Charging == KAL_TRUE) { //LIUJ20151251520ADDO
		FG_dod_1 = gFG_DOD0 - ((gFG_columb * 100) / gFG_BATT_CAPACITY_aging);
		if(FG_dod_1 < 12 && FG_dod_1 > 0 && gFG_voltage>4320) {
			if (timer_ct >= 8)
			{
				FG_dod_1--;
				gFG_DOD0 = FG_dod_1;
				reset_parameter_car();
				bm_print(BM_LOG_CRTI, "[FGRST] up reset FG_dod_1(%d) to pre(%d) \n", FG_dod_1, FG_dod_pre);
				timer_ct = 1;
				FG_dod_pre = FG_dod_1;
			}
			if (FG_dod_pre == FG_dod_1)
			{
				timer_ct++;
			} else {
				timer_ct = 1;
	...
	} else if (bat_is_charger_exist() == KAL_TRUE) { 
                timer_ct = 0;
		FG_dod_1 = gFG_DOD0 - ((gFG_columb * 100) / gFG_BATT_CAPACITY_aging);
        } else {
                timer_ct = 0;
		FG_dod_1 = gFG_DOD0 - ((gFG_columb * 100) / (gFG_BATT_CAPACITY_aging*50/51));   //100/104
	}
	if (bat_is_charger_exist() == KAL_FALSE) { //LIUJ201512251158MODY
		if(FG_dod_1 == 94 || FG_dod_1 == 84 || FG_dod_1 == 74 || FG_dod_1 == 64) {
			kal_int32 dvalue = 0;
			dvalue = fgauge_get_dod0(battery_meter_get_battery_voltage(KAL_TRUE), force_get_tbat(KAL_FALSE), KAL_FALSE);
			if (dvalue > FG_dod_1 && (dvalue - FG_dod_1 >= 1)) {
				if(dvalue - FG_dod_1 >= 2) gFG_DOD0 = FG_dod_1 + 2;
				else gFG_DOD0 = FG_dod_1 + 1;
				gFG_DOD1 = gFG_DOD0;
				reset_parameter_car();
				bm_print(BM_LOG_CRTI, "[FGRST] down reset FG_dod_1(%d) to dvalue(%d) \n", FG_dod_1, dvalue);
				FG_dod_1 = gFG_DOD0;
			}
		}
	}
...
kal_int32 battery_meter_reset(void)
{
	...
	reset_parameter_car();
        #if LOWPRE_FIX_RESET_FG
	if (bat_is_charger_exist() == KAL_TRUE || (bat_is_charger_exist() == KAL_FALSE && ui_percentage > 50 ))  //LIUJ201512241436MODY
        #endif
...
static int battery_meter_resume(struct platform_device *dev)
{
	...
	_g_bat_sleep_total_time_ns+= (rtc_time_after_sleep.tv_nsec - g_rtc_time_before_sleep.tv_nsec+1000000000LL*(rtc_time_after_sleep.tv_sec - g_rtc_time_before_sleep.tv_sec));
	_g_bat_sleep_total_time=_g_bat_sleep_total_time_ns/1000000000LL;// += rtc_time_after_sleep.tv_sec - g_rtc_time_before_sleep.tv_sec;
