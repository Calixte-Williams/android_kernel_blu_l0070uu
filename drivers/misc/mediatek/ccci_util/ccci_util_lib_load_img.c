...
#ifdef CONFIG_PROJECT_WIKO_ONE
	// scan_image_list(md->index, "modem_%d_%s_n.img", tmp_md_img_list, MAX_IMG_NUM);
	printk("===lgc=====scan_image_list==start=%s=\n",type_str[0]);
#endif
...
#ifdef CONFIG_PROJECT_WIKO_ONE
		printk("===lgc=====scan_image_list======%s==%s===\n", CONFIG_MODEM_FIRMWARE_CIP_PATH, img_name);
		printk("===lgc=====scan_image_list======%s=========\n", full_path);
#endif
...
static void get_md_postfix(int md_id, char k[], char buf[], char buf_ex[])
{
...
#ifdef CONFIG_PROJECT_WIKO_ONE
	printk("====lgc================get_md_postfix==start=====\n");
	printk("====lgc================buf=%s=====\n",buf);
#endif
	int X, Ex;
...
#ifdef CONFIG_PROJECT_WIKO_ONE
	printk("====lgc=============5460======================value_RF_ID1=============value_RF_ID2==========================\n");
	s32 value_RF_ID1= -1;
	s32 value_RF_ID2= -1;
	...
	mt_set_gpio_pull_enable(GPIO_FDD_BAND_SUPPORT_DETECT_1ST_PIN, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_FDD_BAND_SUPPORT_DETECT_1ST_PIN, GPIO_PULL_UP);*/
	value_RF_ID1 = mt_get_gpio_in(GPIO_FDD_BAND_SUPPORT_DETECT_1ST_PIN);// 98 16
	value_RF_ID2 = mt_get_gpio_in(GPIO_FDD_BAND_SUPPORT_DETECT_2ND_PIN);//97  15  
	printk("===lgc===value_RF_ID1=%d========k=%s\n",value_RF_ID1,k);
	printk("===lgc===value_RF_ID2=%d========\n",value_RF_ID2);
#endif
	// K
	if(k == NULL)
	{
...
#ifdef CONFIG_PROJECT_WIKO_ONE				
		if(value_RF_ID1==0 && value_RF_ID2==0)
		{
			snprintf(YY_K, IMG_POSTFIX_LEN, "_%s_1", type_str[feature_val]);
		  	printk("===lgc===value_RF_ID==1=l5460_lwg_modem_wiko_multimd_01==wik_eu vn du=====\n");
		}
		else if(value_RF_ID1==0 && value_RF_ID2==1)
		{
			snprintf(YY_K, IMG_POSTFIX_LEN, "_%s_2", type_str[feature_val]);
			printk("===lgc===value_RF_ID2==2=l5460_lwg_modem_wiko_multimd_02=wik_th======\n");
		}
		else if(value_RF_ID1==1 && value_RF_ID2==0)
		{
			snprintf(YY_K, IMG_POSTFIX_LEN, "_%s_3", type_str[feature_val]);
			printk("===lgc===value_RF_ID3==l5460_lwg_modem_wiko_multimd_03==wik_id========\n");
		}
		else
		{
			snprintf(YY_K, IMG_POSTFIX_LEN, "_%s_n", type_str[feature_val]);
			printk("===lgc===value_RF_IDn==other==n========\n");				
		}
#else
		{
			snprintf(YY_K, IMG_POSTFIX_LEN, "_%s_n", type_str[feature_val]);
