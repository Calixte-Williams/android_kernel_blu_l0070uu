	...
	for(k=0;k<e_Max_Part_Num_Per_Dev;k++)
	{
		if(g_pFlashInitFunc[i][j][k]!=0)
		{
		    if(gLowBatDuty[i][j]!=-1)
		    {
		        ...
		        logI("decFlash i,j,k,duty %d %d %d %d \n", i, j, k, duty);
...
static void bat_per_protection_powerlimit_flashlight(BATTERY_PERCENT_LEVEL level)
{
    ...
    {
    if(get_boot_mode() != 4){//factory not run
            closeFlash();
    	}
        ...
        case FLASH_IOC_IS_LOW_POWER:
            logI("FLASH_IOC_IS_LOW_POWER");
            {
                ...
                if(gLowPowerPer!=BATTERY_PERCENT_LEVEL_0 || gLowPowerVbat!=LOW_BATTERY_LEVEL_0 )
					...
					if(get_boot_mode() != 4){// factory not run
                   			 #ifdef CONFIG_PROJECT_L5460_MYP_PH
                   			 isLow=0;
                   			 #else
                   			 isLow=1;
                   			 #endif
                ...
                logI("FLASH_IOC_IS_LOW_POWER %d %d %d \n",gLowPowerPer,gLowPowerVbat,isLow);
                kdArg.arg = isLow;
                if(copy_to_user((void __user *) arg , (void*)&kdArg , sizeof(kdStrobeDrvArg)))
    			{
		...
		case FLASH_IOC_GET_MAIN2_PART_ID:
			{
				...
				logI("FLASH_IOC_GET_PART_ID line=%d partId=%d \n",__LINE__,partId);
			}
          	break;
...
static int flashlight_probe(struct platform_device *dev)
{
    ...
    if (ret) {
        logE("[flashlight_probe] alloc_chrdev_region fail: %d ~ \n", ret);
        goto flashlight_probe_error;
    } else {
        logI("[flashlight_probe] major: %d, minor: %d ~ \n", MAJOR(flashlight_devno), MINOR(flashlight_devno));
    }
    ...
    if (err) {
        logE("[flashlight_probe] cdev_add fail: %d ~ \n", err);
        goto flashlight_probe_error;
    }
#else
